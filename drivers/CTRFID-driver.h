...
/*
 * $Id: CTRFID-driver.h 29 2010-06-25 12:55:41Z xander $
 */

#ifndef __CTRFID_driver_H__
#define __CTRFID_driver_H__

/** \file CTRFID-driver.h
 * \brief Codatex RFID driver
 *
 * CTRFID-driver.h provides an API for the Codatex RFID sensor.
 ...
 * \version 0.2
 * \example CTRFID-test1.c
 * \example CTRFID-test2.c
 */
...
bool _CTRFIDsendCommand(tSensors link, ubyte command) {
  ...
  CTRFID_I2CRequest.arr[0] = 3;                          // Message size
  CTRFID_I2CRequest.arr[1] = CTRFID_I2C_ADDR;            // I2C Address of RFID sensor
  CTRFID_I2CRequest.arr[2] = CTRFID_OFFSET + CTRFID_CMD; // Address of cmd registry
  CTRFID_I2CRequest.arr[3] = command;                    // Mode to set

  return writeI2C(link, CTRFID_I2CRequest, 0);
}
...
bool _CTRFIDsendDummy(tSensors link) {
  ...
  CTRFID_I2CRequest.arr[0] = 1;                          // Message size
  CTRFID_I2CRequest.arr[1] = CTRFID_I2C_ADDR;            // I2C Address of RFID sensor
...
bool _CTRFIDreadStatus(tSensors link, ubyte &_status) {
  ...
  CTRFID_I2CRequest.arr[0] = 2;               // Message size
  CTRFID_I2CRequest.arr[1] = CTRFID_I2C_ADDR; // I2C Address
  CTRFID_I2CRequest.arr[2] = CTRFID_STATUS;   // Address for sensor's status

  if (!writeI2C(link, CTRFID_I2CRequest, 1))
    return false;

  if (!readI2C(link, CTRFID_I2CReply, 1))
    return false;

  _status = CTRFID_I2CReply.arr[0];
...
bool CTRFIDreadTransponder(tSensors link, string &transponderID) {
  ...
  CTRFID_I2CRequest.arr[0] = 2;                                        // Message size
  CTRFID_I2CRequest.arr[1] = CTRFID_I2C_ADDR;                     // I2C Address of RFID sensor
  CTRFID_I2CRequest.arr[2] = CTRFID_OFFSET + CTRFID_BYTE1;   // Address byte1 registry
  if (!writeI2C(link, CTRFID_I2CRequest, 5))
    return false;

  if (!readI2C(link, CTRFID_I2CReply, 5))
    return false;
  ...
  for (int i=0;i<5;i++) {
    ...
    StringFormat(transponderID, "%s%02x", transponderID, CTRFID_I2CReply.arr[i]);
  }
  return true;
}
