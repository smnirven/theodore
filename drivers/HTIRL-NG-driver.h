 ...
 * @{
 * @defgroup htirlng IR Link Sensor NG
 * HiTechnic IR Link Sensor driver NG
 * @{
 */

/*
 * $Id: HTIRL-NG-driver.h 33 2010-09-24 10:11:54Z soldaatx $
 */

#ifndef _HTIRL_H_
#define _HTIRL_H_
/** \file HTIRL-NG-driver.h
 * \brief HiTechnic IR Link Sensor driver (Next Generation)
 *
 * HTIRL-NG-driver.h provides an API for the HiTechnic IR Link Sensor.
 *
 * Changelog:
 * - 1.0: Initial release
 * - 1.1: Minor changes
...
void PFsinglePinOutputMode(tSensors link, byte channel, byte _motor, ePWMMotorCommand _motorCmd) {
  ...
  // This is the unencoded command for the IR receiver
  _iBuffer.arr[0] = (toggle[link] <<7 ) + (channel << 4) + (1 << 2) + _motor;
  _iBuffer.arr[1] = (_motorCmd << 4);
  _iBuffer.arr[1] += 0xF ^ (_iBuffer.arr[0] >> 4) ^ (_iBuffer.arr[0] & 0xF) ^ (_iBuffer.arr[1] >> 4);
...
void PFMotor(tPFmotor pfmotor, ePWMMotorCommand _motorCmd) {
  PFsinglePinOutputMode((tSensors)PFSPORT(pfmotor), PFCHAN(pfmotor), PFMOT(pfmotor), _motorCmd);
}
...
void transmitIR(tSensors link, tByteArray &oBuffer, int channel) {
  ...
  starttime = nPgmTime;
  if (!writeI2C(link, oBuffer, 0)) return;
