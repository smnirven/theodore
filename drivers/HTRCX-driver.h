...
/*
 * $Id: HTRCX-driver.h 36 2010-11-11 10:20:54Z xander $
 */
...
bool HTRCXsendHeader(tSensors link) {
  ...
  HTRCXI2CRequest.arr[0] = 8;
  HTRCXI2CRequest.arr[1] = 0x02;
  HTRCXI2CRequest.arr[2] = 0x4A;
  HTRCXI2CRequest.arr[3] = 0x55;
  HTRCXI2CRequest.arr[4] = 0xFF;
  HTRCXI2CRequest.arr[5] = 0x00;
  HTRCXI2CRequest.arr[6] = 0x03;
  HTRCXI2CRequest.arr[7] = 0x00;
  HTRCXI2CRequest.arr[8] = 0x01;

  return writeI2C(link, HTRCXI2CRequest, 0);
}
...
void HTRCXencode(tSensors link, tByteArray &iBuffer, tByteArray &oBuffer) {
  ...
  oBuffer.arr[0] = 2 + (msgsize * 2) + 2 + 3;
  oBuffer.arr[1] = 0x02;
  oBuffer.arr[2] = 0x4D - ((msgsize * 2) + 2);

  // Build the outgoing IR message and inverse
  // Keep track of checksum
  for (int i = 0; i < msgsize; i++) {
    checksum += iBuffer.arr[i + 1];
    oBuffer.arr[3 + (i * 2)] =  iBuffer.arr[i + 1];
    oBuffer.arr[4 + (i * 2)] = ~iBuffer.arr[i + 1];
    //oBuffer.arr[4 + (i * 2)] = ~oBuffer.arr[3 + (i * 2)];
  }

  // Add checksum to outoing packet
  oBuffer.arr[3 + (msgsize * 2)] =   checksum & 0xFF;
  oBuffer.arr[4 + (msgsize * 2)] = ~(checksum & 0xFF);

  // Generate IR Link info
  oBuffer.arr[3 + (msgsize * 2) + 2] = (msgsize * 2) + 2;
  oBuffer.arr[4 + (msgsize * 2) + 2] = 0x00;
  oBuffer.arr[5 + (msgsize * 2) + 2] = 0x01;
}
...
bool HTRCXreadResp(tSensors link, tByteArray &response) {
  ...
  HTRCXI2CRequest.arr[0] = 2;
  HTRCXI2CRequest.arr[1] = 0x02;
  HTRCXI2CRequest.arr[2] = 0x50;

  if (!writeI2C(link, HTRCXI2CRequest, 16))
    return false;

  if (!readI2C(link, HTRCXI2CReply, 16))
    return false;

  // Print the contents
  // printBuffer(HTRCXI2CReply);

  if ((HTRCXI2CReply.arr[0] > 0) && (HTRCXI2CReply.arr[0] < 16)) {
    memcpy(response, HTRCXI2CReply, HTRCXI2CReply.arr[0]);
  }

  memset(HTRCXI2CRequest, 0, sizeof(tByteArray));
  // Clear the buffer count
  HTRCXI2CRequest.arr[0] = 16;
  HTRCXI2CRequest.arr[1] = 0x02;
  HTRCXI2CRequest.arr[2] = 0x50;
  return writeI2C(link, HTRCXI2CRequest, 0);
}
...
bool HTRCXplaySound(tSensors link, unsigned byte sound) {
  ...
  HTRCXCmdToggle ^= 0x08;
  HTRCXIRMsg.arr[0] = 2;
  HTRCXIRMsg.arr[1] = 0x51 + HTRCXCmdToggle;
  HTRCXIRMsg.arr[2] = sound;

  HTRCXsendHeader(link);
  wait1Msec(12);
  HTRCXencode(link, HTRCXIRMsg, HTRCXI2CRequest);
  writeI2C(link, HTRCXI2CRequest, 0);
  wait1Msec(12);
...
bool HTRCXsendByte(tSensors link, unsigned byte data) {
  HTRCXIRMsg.arr[0] = 2;
  HTRCXIRMsg.arr[1] = 0xF7;   // No need to toggle this, apparently.
  HTRCXIRMsg.arr[2] = data;
...
bool HTRCXsendWord(tSensors link, short data) {
  HTRCXIRMsg.arr[0] = 3;
  HTRCXIRMsg.arr[1] = 0xA2;
  HTRCXIRMsg.arr[2] = data & 0xFF;
  HTRCXIRMsg.arr[3] = (data >> 8) & 0xFF;
...
bool HTRCXmotorOn(tSensors link, unsigned byte _motor) {
  ...
  HTRCXCmdToggle ^= 0x08;
  HTRCXIRMsg.arr[0] = 2;
  HTRCXIRMsg.arr[1] = 0x21 + HTRCXCmdToggle;
  HTRCXIRMsg.arr[2] = _motor + 0x80 + 0x40;
...
bool HTRCXmotorFwd(tSensors link, unsigned byte _motor) {
  ...
  HTRCXCmdToggle ^= 0x08;
  HTRCXIRMsg.arr[0] = 2;
  HTRCXIRMsg.arr[1] = 0xE1 + HTRCXCmdToggle;
  HTRCXIRMsg.arr[2] = _motor + 0x80;
...
bool HTRCXmotorPwr(tSensors link, unsigned byte _motor, unsigned byte power) {
  ...
  HTRCXCmdToggle ^= 0x08;
  HTRCXIRMsg.arr[0] = 3;
  HTRCXIRMsg.arr[1] = 0x13 + HTRCXCmdToggle;
  HTRCXIRMsg.arr[2] = _motor;
  HTRCXIRMsg.arr[3] = 2;
  HTRCXIRMsg.arr[4] = power;
