...
/*
 * $Id: LEGOTMP-driver.h 29 2010-06-25 12:55:41Z xander $
 */

#ifndef __LEGOTMP_DRIVER_H__
#define __LEGOTMP_DRIVER_H__

/** \file LEGOTMP-driver.h
 * \brief RobotC New Temperature Sensor Driver
 *
 * LEGOTMP-driver.h provides an API for the Lego Temperature Sensor.
 *
 * Changelog:
 * - 0.1: Initial release
 ...
 * \version 0.2
 * \example LEGOTMP-test1.c
 * \example LEGOTMP-test2.c
 */
...
bool _LEGOTMPreadConfig(tSensors link, ubyte &config) {
  ...
  LEGOTMP_I2CRequest.arr[0] = 2;                // Message size
  LEGOTMP_I2CRequest.arr[1] = LEGOTMP_I2C_ADDR; // I2C Address
  LEGOTMP_I2CRequest.arr[2] = LEGOTMP_CONFIG;   // Value address

  if (!writeI2C(link, LEGOTMP_I2CRequest, 1))
    return false;

  if (!readI2C(link, LEGOTMP_I2CReply, 1))
    return false;

  config = LEGOTMP_I2CReply.arr[0];
...
bool _LEGOTMPsetConfig(tSensors link, ubyte &config) {
  ...
  LEGOTMP_I2CRequest.arr[0] = 3;                // Message size
  LEGOTMP_I2CRequest.arr[1] = LEGOTMP_I2C_ADDR; // I2C Address
  LEGOTMP_I2CRequest.arr[2] = LEGOTMP_CONFIG;   // Value address
  LEGOTMP_I2CRequest.arr[3] = config;           // Value to be set

  return writeI2C(link, LEGOTMP_I2CRequest, 0);
}
...
bool LEGOTMPreadTemp(tSensors link, float &temp) {
  ...
  LEGOTMP_I2CRequest.arr[0] = 2;                // Message size
  LEGOTMP_I2CRequest.arr[1] = LEGOTMP_I2C_ADDR; // I2C Address
  LEGOTMP_I2CRequest.arr[2] = LEGOTMP_TEMP;     // Value address

  if (!writeI2C(link, LEGOTMP_I2CRequest, 2))
    return false;

  if (!readI2C(link, LEGOTMP_I2CReply, 2))
    return false;

  b1 = ubyteToInt(LEGOTMP_I2CReply.arr[0]);

  switch (_LEGOTMPconvertAccuracy(config)) {
    case A_MIN:
      ///128 to have only the most significant bit (9 bits accuracy - 8 (b1) = 1 bit to keep)
      b2 = (ubyteToInt(LEGOTMP_I2CReply.arr[1]) >> 7) * 0.5;
      break;
    case A_MEAN1:
      ///64 to have only the 2 most significant bits
      b2 = (ubyteToInt(LEGOTMP_I2CReply.arr[1]) >> 6) * 0.25;
      break;
    case A_MEAN2:
      ///32 to have only the 3 most significant bits
      b2 = (ubyteToInt(LEGOTMP_I2CReply.arr[1]) >> 5) * 0.125;
      break;
    case A_MAX:
      ///16 to have only the 4 most significant bits
      b2 = (ubyteToInt(LEGOTMP_I2CReply.arr[1]) >> 4) * 0.0625;
      break;
  }
