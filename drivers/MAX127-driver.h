...
/*
 * $Id: MAX127-driver.h 29 2010-06-25 12:55:41Z xander $
 */

#ifndef __MAX127_H__
#define __MAX127_H__
/** \file MAX127-driver.h
...
int MAX127readChan(tSensors link, byte i2caddress, byte adcchannel) {
  ...
  MAX127_I2CRequest.arr[0] = 2;                            // Message size
  MAX127_I2CRequest.arr[1] = i2caddress;                   // I2C Address
  MAX127_I2CRequest.arr[2] = (1 << 7) + (adcchannel << 4); // Control Byte
  // Control byte is calculated as follows:
  // start bit (7) + channel number bit-shifted into place

  if (!writeI2C(link, MAX127_I2CRequest, 2))
    return -1;

  if (!readI2C(link, MAX127_I2CReply, 2))
    return -1;
  ...
  _chVal = (ubyteToInt(MAX127_I2CReply.arr[0]) << 4) + (ubyteToInt(MAX127_I2CReply.arr[1]) >> 4);
