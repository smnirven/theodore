...
/*
 * $Id: MSLL-driver.h 29 2010-06-25 12:55:41Z xander $
 */
 ...
 *
 * MSLL-driver.h provides an API for the Mindsensors Line Tracking Sensor.
 *
 * Changelog:
 *  - 0.1 Initial	TFR<br>
 *  - 0.2 Added ability to read multi-byte values for raw, white and black thresholds.<br>
 ...
 * \version 0.6
 * \example MSLL-test2.c
 * \example MSLL-test3.c
 * \example MSLL-test4.c
 */
...
#define LL_SENSOR_RAW 		0X49  /*!< byte array (8) with raw value for each sensor */
#define LL_WHITE_LIMIT 		0X51  /*!< byte array (8) with raw value of white calibration for each sensor */
#define LL_BLACK_LIMIT		0X59  /*!< byte array (8) with raw value of black calibration for each sensor */
#define LL_SENSOR_UNCAL   0x74  /*!< byte array (16) with uncalibrated sensor data */
...
bool LLsetPoint(tSensors link, byte data);                /*!< WRITE mid-point or center of line value */
int LLsetPoint(tSensors link);                            /*!< READ SetPoint value */

bool LLsetKp(tSensors link, byte data, byte factor);      /*!< WRITE Kp value */
int LLreadKp(tSensors link);                              /*!< READ Kp value */
int LLreadKpFactor(tSensors link);                        /*!< READ p factor value */

bool LLsetKi(tSensors link, byte data, byte factor);      /*!< WRITE Ki value */
int LLreadKi(tSensors link);                              /*!< READ Ki value */
int LLreadKiFactor(tSensors link);                        /*!< READ i factor value */

bool LLsetKd(tSensors link, byte data, byte factor);      /*!< WRITE Kd value */
...
//*******************************************************************************
bool _lineLeader_cmd(tSensors link, byte cmd);                      /*!< Send a command to the Line Leader */
bool _lineLeader_write(tSensors link, byte regToWrite, byte data);  /*!< Write to a Line Leader Register */
bool _lineLeader_read(tSensors link, byte regToRead, byte &retval); /*!< Read one byte from a Line Leader Register */
bool _lineLeader_read(tSensors link, byte regToRead, int numBytes, tByteArray &pDataMsg);  /*!< Read data from a Line Leader Register */
...
PRELIMINARY COMMANDS FROM NXC LIB CODE
 ...
 - W White balance
 */
bool _lineLeader_cmd(tSensors link, byte cmd) {
  LL_I2CRequest.arr[0] = 3;             // Message size
  LL_I2CRequest.arr[1] = LL_I2C_ADDR;   // I2C Address
  LL_I2CRequest.arr[2] = LL_CMD_REG;    // Register used for issuing commands
  LL_I2CRequest.arr[3] = cmd;           // Command to be executed

  return writeI2C(link, LL_I2CRequest, 0);
}
...
bool _lineLeader_write(tSensors link, byte regToWrite, byte data) {

  memset(LL_I2CRequest, 0, sizeof(tByteArray));
	LL_I2CRequest.arr[0] = 3;             // Message size
  LL_I2CRequest.arr[1] = LL_I2C_ADDR;   // I2C Address
  LL_I2CRequest.arr[2] = regToWrite;    // Register address to set
  LL_I2CRequest.arr[3] = data;          // value to place in register address

  if (!writeI2C(link, LL_I2CRequest, 0))
		return false;
	return true;
}
...
/**
 * This function reads one byte from a register in the LL sensor over I2C.
 *
 * Note: this is an internal function and should not be called directly.
 * @param link the sensor port number
 * @param regToRead the register to read from
 * @param retval the byte in which to store the reply
 * @return true if no error occured, false if it did
 */
bool _lineLeader_read(tSensors link, byte regToRead, byte &retval) {
	memset(LL_I2CRequest, 0, sizeof(LL_I2CRequest));
	LL_I2CRequest.arr[0] =  2;
	LL_I2CRequest.arr[1] =  LL_I2C_ADDR;
	LL_I2CRequest.arr[2] =  regToRead;

  if (!writeI2C(link, LL_I2CRequest, 1))
    return false;

  if (!readI2C(link, LL_I2CReply, 1))
    return false;

  retval = ubyteToInt(LL_I2CReply.arr[0]);
  return true;
}
 ...
 * @return true if no error occured, false if it did
 */
bool _lineLeader_read(tSensors link, byte regToRead, int numBytes, tByteArray &pDataMsg) {
	memset(LL_I2CRequest, 0, sizeof(tByteArray));
  ...
  if (!writeI2C(link, LL_I2CRequest, numBytes))
    return false;

  if (!readI2C(link, LL_I2CReply, numBytes))
    return false;
 ...
 * @return true if no error occured, false if it did
 */
bool LLsetPoint(tSensors link, byte data){
 ...
 * @return true if no error occured, false if it did
 */
bool LLsetKp(tSensors link, byte data, byte factor){
 ...
 * @return true if no error occured, false if it did
 */
bool LLsetKi(tSensors link, byte data, byte factor){
 ...
 * @return true if no error occured, false if it did
 */
bool LLsetKd(tSensors link, byte data, byte factor){
...
/**
 * Read a byte with each bit equal to a sensor.<br>
 * 1 = Line<br>
 * 0 = No Line<br>
 * <pre> SENSOR:      0    1    2    3    4    5    6    7
...
bool LLreadSensorUncalibrated (tSensors link, tIntArray &sensorValues) {
  ...
  for (int i = 0; i < 8; i++) {
    sensorValues.arr[i] = (0xFF & sensorData.arr[(i*2)]) + ((0xFF & sensorData.arr[((i*2)+1)]) << 8);
  }
