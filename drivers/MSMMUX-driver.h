...
/*
 * $Id: MSMMUX-driver.h 29 2010-06-25 12:55:41Z xander $
 */
 ...
 *
 * MSMMUX-driver.h provides an API for the Mindsensors Motor MUX.
 *
 * Changelog:
 * - 0.1: Initial release
 *
...
bool MSMMUXreadStatus(tMUXmotor muxmotor, ubyte &motorStatus) {
  ...
  MSMMUX_I2CRequest.arr[0] = 2;               // Message size
  MSMMUX_I2CRequest.arr[1] = MSMMUX_I2C_ADDR; // I2C Address

  switch ((byte)MPORT(muxmotor)) {
    case 0: MSMMUX_I2CRequest.arr[2] = MSMMUX_STATUS_MOT1; break;
    case 1: MSMMUX_I2CRequest.arr[2] = MSMMUX_STATUS_MOT2; break;
  }

  if (!writeI2C((tSensors)SPORT(muxmotor), MSMMUX_I2CRequest, 1))
    return false;

  if (!readI2C((tSensors)SPORT(muxmotor), MSMMUX_I2CReply, 1))
    return false;

  motorStatus = MSMMUX_I2CReply.arr[0];
...
bool MSMMUXsendCommand(tSensors link, ubyte channel, long setpoint, byte speed, ubyte seconds, ubyte commandA) {
  ...
  MSMMUX_I2CRequest.arr[0] = 10;               // Message size
  MSMMUX_I2CRequest.arr[1] = MSMMUX_I2C_ADDR; // I2C Address
  MSMMUX_I2CRequest.arr[2] = MSMMUX_MOT_OFFSET + (channel * MSMMUX_ENTRY_SIZE);
  MSMMUX_I2CRequest.arr[3] = (setpoint >>  0) & 0xFF;
  MSMMUX_I2CRequest.arr[4] = (setpoint >>  8) & 0xFF;
  MSMMUX_I2CRequest.arr[5] = (setpoint >> 16) & 0xFF;
  MSMMUX_I2CRequest.arr[6] = (setpoint >> 24) & 0xFF;
  MSMMUX_I2CRequest.arr[7] = (speed & 0xFF);
  MSMMUX_I2CRequest.arr[8] = seconds;
  MSMMUX_I2CRequest.arr[9] = 0;
  MSMMUX_I2CRequest.arr[10] = commandA;
  ...
  return writeI2C(link, MSMMUX_I2CRequest, 0);
...
bool MSMMUXsendCommand(tSensors link, ubyte command) {
  ...
  MSMMUX_I2CRequest.arr[0] = 3;               // Message size
  MSMMUX_I2CRequest.arr[1] = MSMMUX_I2C_ADDR; // I2C Address
  MSMMUX_I2CRequest.arr[2] = MSMMUX_REG_CMD;
  MSMMUX_I2CRequest.arr[3] = command;
...
bool MSMMUXsetPID(tSensors link, int kpTacho, int kiTacho, int kdTacho, int kpSpeed, int kiSpeed, int kdSpeed, ubyte passCount, ubyte tolerance) {
  ...
  MSMMUX_I2CRequest.arr[0] = 16;               // Message size
  MSMMUX_I2CRequest.arr[1] = MSMMUX_I2C_ADDR; // I2C Address
  MSMMUX_I2CRequest.arr[2] = MSMMUX_KP_TACHO;
  MSMMUX_I2CRequest.arr[3] = kpTacho & 0xFF;
  MSMMUX_I2CRequest.arr[4] = (kpTacho >> 8) & 0xFF;
  MSMMUX_I2CRequest.arr[5] = kiTacho & 0xFF;
  MSMMUX_I2CRequest.arr[6] = (kiTacho >> 8) & 0xFF;
  MSMMUX_I2CRequest.arr[7] = kdTacho & 0xFF;
  MSMMUX_I2CRequest.arr[8] = (kdTacho >> 8) & 0xFF;
  MSMMUX_I2CRequest.arr[9] = kpSpeed & 0xFF;
  MSMMUX_I2CRequest.arr[10] = (kpSpeed >> 8) & 0xFF;
  MSMMUX_I2CRequest.arr[11] = kiSpeed & 0xFF;
  MSMMUX_I2CRequest.arr[12] = (kiSpeed >> 8) & 0xFF;
  MSMMUX_I2CRequest.arr[13] = kdSpeed & 0xFF;
  MSMMUX_I2CRequest.arr[14] = (kdSpeed >> 8) & 0xFF;
  MSMMUX_I2CRequest.arr[15] = passCount;
  MSMMUX_I2CRequest.arr[16] = tolerance;
...
bool MSMMotor(tMUXmotor muxmotor, byte power) {
  ...
  switch (mmuxData[SPORT(muxmotor)].targetUnit[MPORT(muxmotor)]) {
    case MSMMUX_ROT_UNLIMITED: return MSMMUXsendCommand((tSensors)SPORT(muxmotor), (ubyte)MPORT(muxmotor), 0, power, 0, commandA);
    case MSMMUX_ROT_DEGREES:   return MSMMUXsendCommand((tSensors)SPORT(muxmotor), (ubyte)MPORT(muxmotor), mmuxData[SPORT(muxmotor)].target[MPORT(muxmotor)], power, 0, commandA);
    case MSMMUX_ROT_SECONDS:   return MSMMUXsendCommand((tSensors)SPORT(muxmotor), (ubyte)MPORT(muxmotor), 0, power, mmuxData[SPORT(muxmotor)].target[MPORT(muxmotor)], commandA);
  }
  return true;
}
...
bool MSMotorStop(tMUXmotor muxmotor) {
  if (MPORT(muxmotor) == 0)
    return MSMotorStop(muxmotor, mmuxData[SPORT(muxmotor)].brake[MPORT(muxmotor)]);
...
bool MSMotorStop(tMUXmotor muxmotor, bool brake) {
  if (MPORT(muxmotor) == 0)
    return MSMMUXsendCommand(MSMMUX, brake ? MSMMUX_CMD_BRAKE_MOT1 : MSMMUX_CMD_FLOAT_MOT1);
  else if (MPORT(muxmotor) == 1)
    return MSMMUXsendCommand(MSMMUX, brake ? MSMMUX_CMD_BRAKE_MOT2 : MSMMUX_CMD_FLOAT_MOT2);
  return true;
}
...
long MSMMotorEncoder(tMUXmotor muxmotor) {
  ...
  switch ((byte)MPORT(muxmotor)) {
    case 0: MSMMUX_I2CRequest.arr[2] = MSMMUX_TACHO_MOT1; break;
    case 1: MSMMUX_I2CRequest.arr[2] = MSMMUX_TACHO_MOT2; break;
  }

  if (!writeI2C((tSensors)SPORT(muxmotor), MSMMUX_I2CRequest, 4))
    return 0;

  if (!readI2C((tSensors)SPORT(muxmotor), MSMMUX_I2CReply, 4))
    return 0;

  result = MSMMUX_I2CReply.arr[0] + (MSMMUX_I2CReply.arr[1]<<8) + (MSMMUX_I2CReply.arr[2]<<16) + (MSMMUX_I2CReply.arr[3]<<24);
...
bool MSMMotorEncoderReset(tMUXmotor muxmotor) {
  switch((byte)MPORT(muxmotor)) {
    case 0: return MSMMUXsendCommand((tSensors)SPORT(muxmotor), MSMMUX_CMD_RESET_MOT1); break;
    case 1: return MSMMUXsendCommand((tSensors)SPORT(muxmotor), MSMMUX_CMD_RESET_MOT2); break;
  }
  return false;
}
 ...
 */
bool MSMMotorEncoderResetAll(tSensors link) {
  return MSMMUXsendCommand(link, MSMMUX_CMD_RESET_ALL);
}
...
bool MSMMotorBusy(tMUXmotor muxmotor) {
  ...
  MSMMUX_I2CRequest.arr[0] = 2;               // Message size
  MSMMUX_I2CRequest.arr[1] = MSMMUX_I2C_ADDR; // I2C Address
  MSMMUX_I2CRequest.arr[2] = MSMMUX_MOT_OFFSET + (MPORT(muxmotor) * MSMMUX_ENTRY_SIZE) + MSMMUX_CMD_A;
  ...
  commandA = MSMMUX_I2CReply.arr[0];

  // If commandA is 0 then the motor can't be busy.
  if (commandA == 0)
    return false;

  if (!MSMMUXreadStatus(muxmotor, status))
    return false;
