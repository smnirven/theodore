...
/*
 * $Id: MSNP-driver.h 40 2011-01-03 09:37:09Z xander $
 */
 ...
 *
 * MSNP-driver.h provides an API for the Mindsensors Numeric Keypad Sensor.\n
 *
 * Changelog:
 * - 0.1: Initial release
 *
...
const byte MSNP_NumMap[] = { -1, 9, 6, 3, 0, 8, 2, 5, -2, 7, 1, 4};
...
// Must be called at the beginning of every power cycle
  ...
  if (!writeI2C(link, MSNP_I2CRequest, 0))
    return false;
...
bool MSNPscanKeys(tSensors link, int &pressedKeys, unsigned byte &key, int &number) {
  ...
  MSNP_I2CRequest.arr[0] = 2;
  MSNP_I2CRequest.arr[1] = MSNP_I2C_ADDR;
  MSNP_I2CRequest.arr[2] = MSNP_DATA_REG;

	if (!writeI2C(link, MSNP_I2CRequest, 2))
	  return false;

	if (!readI2C(link, MSNP_I2CReply, 2))
	  return false;

  pressedKeys = MSNP_I2CReply.arr[0] + (MSNP_I2CReply.arr[1] * 256);
...
int MSNPscanKeys(tSensors link) {
  ...
  keyPress = MSNP_I2CReply.arr[0] + (MSNP_I2CReply.arr[1] * 256);
