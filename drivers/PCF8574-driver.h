...
/*
 * $Id: PCF8574-driver.h 29 2010-06-25 12:55:41Z xander $
 */
 ...
 *
 * PCF8574-driver.h provides an API for the Philips PCF8574 IO MUX.
 *
 * Changelog:
 * - 0.1: Initial release
 *
...
bool PCF8574sendBytes(tSensors link, ubyte _byte) {
  ...
  PCF8574_I2CRequest.arr[0] = 2;               // Message size
  PCF8574_I2CRequest.arr[1] = PCF8574_I2C_ADDR; // I2C Address
  PCF8574_I2CRequest.arr[2] = _byte;

  return writeI2C(link, PCF8574_I2CRequest, 0);
}
...
bool PCF8574readBytes(tSensors link, ubyte &_byte) {
  ...
  PCF8574_I2CRequest.arr[0] = 1;               // Message size
  PCF8574_I2CRequest.arr[1] = PCF8574_I2C_ADDR; // I2C Address
  PCF8574_I2CRequest.arr[2] = _byte;

  if (!writeI2C(link, PCF8574_I2CRequest, 1))
    return false;

  if (!readI2C(link, PCF8574_I2CReply, 1))
    return false;

  _byte = PCF8574_I2CReply.arr[0];
